...
class PreMarketService(ScriptedService):
    ...
    def on_complete(self, uid, draft, store, line, reply_token=""):
        ...
        try:
            ...
            if candle_data:
                # 從 K 線資料中提取昨日收盤價（盤前使用歷史最後一筆），yfinance 失敗則用 Fugle
                current_price = 0.0
                try:
    ...
    def _fetch_candle_data(self, stock_id: str, premarket: bool = False) -> Optional[str]:
        ...
        try:
            ...
            print(f"[pre_market] yfinance K線取得失敗，改用 Fugle：{e}")
        ...
        if df is None or len(df) == 0:
            try:
                ...
                print(f"[pre_market] Fugle K線取得也失敗：{e}")
