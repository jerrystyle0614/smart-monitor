...
class PostMarketService(ScriptedService):
    ...
    def _format_analysis_message(
        ...
        if technical:
            msg_parts.append("🔍 技術面回顧")
            if isinstance(technical, dict):
                msg_parts.append(f"- 今日走勢：{technical.get('trend', '未知')}")
                support = technical.get("support")
                if support:
                    msg_parts.append(f"- 支撐價位：{support}")
                resistance = technical.get("resistance")
                if resistance:
                    msg_parts.append(f"- 壓力價位：{resistance}")
            ...
            if isinstance(entry_exit, dict):
                ...
                if targets:
                    if isinstance(targets, dict):
                        msg_parts.append(
                            f"- 目標價位：短期 {targets.get('short_term')} / "
